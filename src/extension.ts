...
import { RpnCompletionItemProvider } from './provider/rpncompletionitemprovider';
import { RpnDiagnostic } from './diagnostic/rpndiagnostic';
...
export function activate(context: vscode.ExtensionContext) {
    ...
    const selector = [
        { language: 'hp42s', scheme: 'file', pattern: '**/*.hp42s' },
        { language: 'free42', scheme: 'file', pattern: '**/*.free42' }
    ];

    const disposableCompletionItemProvider = vscode.languages.registerCompletionItemProvider(selector, new RpnCompletionItemProvider(),'"',' ');

    const diagnostics = new RpnDiagnostic();
